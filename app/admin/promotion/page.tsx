"use client";
import "@/style/admin-promotion.css";
...
import {
  PromotionReqSchema,
  PromotionReq,
} from "@/schema/request/message/Promotion";
...
export default function AdminPromotionPage() {
  ...
  const [filterType, setFilterType] = useState<"ALL" | "PERCENT" | "AMOUNT">(
    "ALL",
  );
  ...
  const t = {
    ...
    subtitle:
      lang === "VI"
        ? "Xem, kích hoạt các chiến dịch ưu đãi, tạo mới mã coupon giảm giá và theo dõi trạng thái."
        : "View active promo campaigns, activate coupon structures, monitor statuses, and register new discount events.",
    searchPlaceholder:
      lang === "VI" ? "Tìm mã, tiêu đề..." : "Search code, campaign title...",
    ...
    drawerTitle:
      lang === "VI" ? "Tạo khuyến mãi mới" : "Register Campaign Invitation",
    ...
    submitBtn:
      lang === "VI" ? "Đăng ký khuyến mãi" : "Register Discount Privilege",
    ...
    noCampaignDesc:
      lang === "VI"
        ? "Nhấn nút tạo khuyến mãi mới ở góc phải để bắt đầu."
        : "Click the Create Campaign button to register your first promo event.",
  };
      ...
      if (res?.data?.code === 200 || res?.data?.data) {
        toast.success(
          lang === "VI"
            ...
            : "Privilege campaign registered successfully!",
        );
        reset();
        setDrawerOpen(false);
        loadData(true);
        ...
        {[
          {
            title: t.totalPromo,
            value: totalCount,
            subtitle: "Total campaigns registered",
            icon: Ticket,
          ...
          },
          {
            title: t.activePromo,
            value: activeCount,
            subtitle: "Currently active on frontend",
            icon: CheckCircle2,
            color: "text-emerald-600 dark:text-emerald-500",
          },
          {
            title: t.percentPromo,
            value: percentCount,
            subtitle: "Percentage voucher structures",
            icon: Percent,
            color: "text-blue-600",
          },
          {
            title: t.amountPromo,
            value: amountCount,
            subtitle: "Flat rate coupon structures",
            icon: DollarSign,
            color: "text-rose-600",
          },
        ].map((c, idx) => {
          ...
          return (
            <div
              key={idx}
              className="glass-promo-card rounded-2xl p-6 relative overflow-hidden"
            >
      ...
      <div className="relative z-10 space-y-6">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-stone-400 border border-dashed border-stone-250 dark:border-stone-850 rounded-[30px] bg-white/20 dark:bg-stone-900/10">
            ...
            <h4 className="text-xs font-bold uppercase tracking-widest text-stone-700 dark:text-stone-300">
              {t.noCampaign}
            </h4>
              ...
              return (
                ...
                >
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                      <span
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[8.5px] font-bold uppercase tracking-wider ${
                          p.status
                      ...
                      <span
                        className={`px-2.5 py-1 text-[9px] font-bold rounded-lg uppercase tracking-wider font-mono flex items-center gap-1.5 ${
                          isPercent
                    ...
                    <div className="p-3 bg-stone-100/50 dark:bg-stone-950/40 border border-stone-200/40 dark:border-stone-850/40 rounded-xl flex items-center justify-between">
                      ...
                      <span className="font-mono text-sm font-extrabold tracking-widest text-stone-900 dark:text-stone-100 bg-amber-500/15 px-2.5 py-0.5 rounded-md border border-amber-500/20">
                        {p.code}
                      </span>
                    </div>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <p className="text-[10px] text-stone-400 dark:text-stone-500 leading-relaxed font-semibold">
                        {p.description}
                      </p>
                    </div>
                  </div>
            ...
            <div className="flex-1 flex flex-col overflow-y-auto">
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-5">
                ...
                <div className="space-y-1.5">
                  ...
                  {errors.value && (
                    <p className="text-red-500 text-[10px] font-bold">
                      {errors.value.message || "Value is required."}
                    </p>
                  )}
                </div>
                ...
                <div className="flex items-center space-x-3 p-4 bg-stone-100/50 dark:bg-stone-950/30 border border-stone-200/20 dark:border-stone-850/20 rounded-2xl hover:border-yellow-600/25 transition-all cursor-pointer">
                  ...
                  <label
                    htmlFor="status"
                    className="text-xs font-bold text-stone-700 dark:text-stone-300 cursor-pointer select-none"
                  >
                    {t.formStatus}
                  </label>
                </div>
